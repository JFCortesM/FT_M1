...
- cacheFunction debe retornar una función. Esta función debe aceptar un argumento (arg) e invocar a cb con ese argumento; hecho eso, debe guardar el argumento junto con el resultado de la invocación 
...
la próxima vez que reciba el mismo argumento, no sea necesario volver a invocar a cb, porque el resultado estará guardado en la "memoria caché".
  ...
  function cacheFunction(cb) {
    ...
    const cache = {};  // creamos la memoria cache por medio del metodo objeto
  
    return function(arg) {  // retornamos la funcion con el parametro 'arg'
      if (cache.hasOwnProperty(arg)) {  //  buscamos por medio del if si el argumento ya fue guardado en la memoria por medio del metodo 'hasOwnProperty()'
        return cache[arg];  // retornamos el resultado
      } else {  // sino
         const result = cb(arg); // hacemos la operacion la cual es invocar a cb con el argumento y guardarlo en una variable resultado
        cache[arg] = result; // agregamos ese resultado al objeto memoria cache
        return result; // y lo retornamos 
      }
    };
  }
